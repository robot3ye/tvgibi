...
export default function ChannelListModal({ isOpen, onClose, channels, currentChannelId }: ChannelListModalProps) {
                ...
                <div className="overflow-y-auto p-4 space-y-2">
                    {channels.map((channel, index) => {
                        ...
                        return (
                            <Link 
                                key={channel.id}
                                href={`/admin/schedule?channel=${channel.id}`}
                                onClick={() => {
                                    // When clicking a link that just changes search params on the same page,
                                    // we can also force a hard navigation or let Next.js handle it.
                                    // In Next.js app router, query param changes don't always trigger a full remount.
                                    // We will rely on the parent page's effect listening to URL changes.
                                    onClose();
                                }}
